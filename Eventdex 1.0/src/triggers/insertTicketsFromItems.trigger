...
system.debug('RRRRRRRRRRRRRRRRRRR '+BLN_EventsConTest.isFirstRun());
 if(BLN_EventsConTest.isFirstRun()){
    ...
    if(trigger.isAfter){
        ...
        MAP<ID, BLN_Item__c> newItems = new MAP<ID, BLN_Item__c>([Select b.service_fee__c, b.sale_start__c, b.sale_end__c, b.price__c, b.min_per_order__c, b.max_per_order__c, 
                        ...
                        b.Item_Type__r.Item_Type_Number__c, b.Currency_Code__c, Item_Pool__r.Badge_Label__c  From BLN_Item__c b WHERE ID IN: newitemsMAP.Keyset() ]);
            ...
            LIST<Ticket__c> savedPackageTickets = [SELECT ID, NAME,Item__r.Name, Item__c, Item_Pool__c, Item_Pool__r.Name , Item_Type__c, Parent_ID__c, Parent_ID__r.Name,  Event__c, Badge_Label__c FROM Ticket__c WHERE ID IN: savedPackagesTicketsId ];
            ...
            LIST<Item_Pool__c> packageLineItems = [SELECT ID, NAME,Addon_Parent__r.Name, Addon_Parent__c, Addon_Count__c, Item_Count__c, Badge_Label__c FROM Item_Pool__c WHERE Addon_Parent__c IN: newPackageItemIds ];
 ...
 MAP<ID, BLN_Item__c> newItems = new MAP<ID, BLN_Item__c>([Select b.service_fee__c, b.sale_start__c, b.sale_end__c, b.price__c, b.min_per_order__c, b.max_per_order__c, 
            ...
            List<Ticket__c> deleteChildPackageTickets=new LIST<Ticket__c>();
    ...
    //We are inserting only IndividualTickets and main Package Items..................................    
              ...
              if(newItems.containsKey(ite.id)){
                    ...
                    if(newItems.get(ite.id).item_type__r.Item_Type_Number__c !=  BLN_Event_UtilityCon.packageItemType()){
                        ...
                        Integer delqty=Integer.valueOf(remquantity)*-1;
                           deleteIndividualTickets.addAll([select id,name,Item__c,Ticket_Status__c from Ticket__c where Item__c=:ite.id and Event__c=:ite.Event__c and Ticket_Status__c='Available' limit :delqty]);
                        ...
                        {
                        Integer delqty=Integer.valueOf(remquantity)*-1;
                           deleteMainPackageTickets.addAll([select id,name,Item__c,Ticket_Status__c from Ticket__c where Item__c=:ite.id and Event__c=:ite.Event__c and Ticket_Status__c='Available' and Parent_ID__c=null limit :delqty]);
                           LIST<ID> deletemainTicketsId = new LIST<ID>();
            for(Ticket__c t: deleteMainPackageTickets){
                deletemainTicketsId .add(t.id);
            }
                         deleteChildPackageTickets.addAll([select id,name,Parent_ID__c from Ticket__c where Parent_ID__c IN :deletemainTicketsId]);
                        }
            ...
            LIST<DataBase.Saveresult> individualTicketsResult = DataBase.insert(insertIndividualTickets, FALSE);
              LIST<DataBase.Deleteresult> deleteTicketsResult = DataBase.delete(deleteIndividualTickets, FALSE);
                      LIST<DataBase.Saveresult> packageTicketsResult =    DataBase.insert(insertMainPackageTickets, FALSE);
                            ...
                            LIST<DataBase.Deleteresult> deleteChildPackageTicketsResult = DataBase.delete(deleteChildPackageTickets, FALSE); 
            ...
            for(Ticket__c tc:[select id,name,Ticket_Status__c,Parent_ID__c,Item__c from Ticket__c where Ticket_Status__c='Available' and Parent_ID__c=null and Item__c=:trigger.newmap.keyset()])
            {
               savedPackagesTicketsId.add(tc.id);
            }
            
            List<Ticket__c> delchild=[select id,name,Ticket_Status__c,Parent_ID__c,Item__c from Ticket__c where Ticket_Status__c='Available' and Parent_ID__c IN:savedPackagesTicketsId];
